...
use combinators as cmb;
use combinators::{OrValue, Parser};

use crate::ast::Node;

use self::combinators::constant;
        ...
        cmb::bind(
            ...
            move |args| {
                cmb::and(
                    ...
                    constant(Node::Call(name.clone(), args)),
                )
            },
        )
    })
...
pub fn make_atom_parser<'a>() -> impl Parser<'a, Node> {
    cmb::bind(
                ...
                cmb::or(
                    ...
                    cmb::and(
                        ...
                        cmb::bind(make_expression_parser(), |e| {
                            cmb::and(make_right_paren_parser(), constant(e))
                        }),
                    ),
                ),
            ),
        ...
        |e| match e {
            OrValue::Lhs(call) => constant(call),
            OrValue::Rhs(id_or_number_or_expr) => match id_or_number_or_expr {
                OrValue::Lhs(id) => constant(Node::Identifier(id)),
                OrValue::Rhs(number_or_expr) => match number_or_expr {
                    OrValue::Lhs(number) => constant(number),
                    OrValue::Rhs(expr) => constant(expr),
                },
            },
        },
    )
